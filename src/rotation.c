...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/22 23:53:50 by iostancu          #+#    #+#             */
/*   Updated: 2022/12/16 21:01:10 by iostancu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_on_y(t_fdf *fdf, t_vec2 *start, t_vec2 *end, t_depth *depth)
{
	...
	start->x = (rot_start.x * cosf(fdf->control.rot_angle))
		- (rot_start.y * sinf(fdf->control.rot_angle));
	start->y = (rot_start.y * cosf(fdf->control.rot_angle))
		+ (rot_start.x * sinf(fdf->control.rot_angle));
	end->x = (rot_end.x * cosf(fdf->control.rot_angle))
		- (rot_end.y * sinf(fdf->control.rot_angle));
	end->y = (rot_end.y * cosf(fdf->control.rot_angle))
		+ (rot_end.x * sinf(fdf->control.rot_angle));
	depth->z = rot_depth.z;
...
void	rotate_top_on_y_3d(t_fdf *fdf, t_bresen *point, t_vec2 *end)
{
	...
	point->start.x = rot_start.x * cos(1) - rot_start.y
		* sin(fdf->control.rot_angle);
	point->start.y = rot_start.y * cos(fdf->control.rot_angle)
		+ rot_start.x * sin(45);
...
void	rotate_top_on_y(t_fdf *fdf, t_bresen *point, t_vec2 *end)
{
	...
	point->start.y = rot_start.y * sin(fdf->control.rot_angle) + rot_start.x;
	end->x = rot_end.x - rot_end.y * sin(fdf->control.rot_angle);
	end->y = rot_end.y * sin(fdf->control.rot_angle) + rot_end.x;
	point->depth.z = rot_depth.z;
	point->depth.z1 = rot_depth.z1;
}
...
void	rotate_point(t_fdf *fdf, t_bresen *point, t_vec2 *end)
{
	...
	rot_depth.z = point->depth.z;
	rot_depth.z1 = point->depth.z1;
	point->start.x = rot_start.x * cosf(fdf->control.rot_angle)
		+ rot_start.y * sinf(fdf->control.rot_angle);
	point->start.y = rot_start.y * cosf(fdf->control.rot_angle)
		- rot_start.x * sinf(fdf->control.rot_angle);
	end->x = rot_end.x * cosf(fdf->control.rot_angle) + rot_end.y
		* sinf(fdf->control.rot_angle);
	end->y = rot_end.y * cosf(fdf->control.rot_angle) - rot_end.x
		* sinf(fdf->control.rot_angle);
	point->depth.z = rot_depth.z;
	point->depth.z1 = rot_depth.z1;
}
